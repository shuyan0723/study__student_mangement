...
// 数据脱敏函数
function desensitizeGradeData(grades: any[]) {
...
function buildAnalysisPrompt(
  desensitizedData: any[],
  dimensions: string[],
  customInstruction?: string,
  courseInfo?: { courseNames: string[]; examType?: string }
): string {
...
// 调用Kimi API
async function callKimiAPI(apiConfig: ApiConfig, prompt: string): Promise<{
  content: string;
  tokensUsed: number;
}> {
  try {
...
// 检查教师调用限额
async function checkTeacherQuota(teacherId: string): Promise<{ hasQuota: boolean; dailyLimit: number }> {
  ...
  // 获取API配置（优先使用数据库配置）
  let apiConfig = await ApiConfig.findOne({ where: { isActive: true } });
  ...
  // 统计今日调用次数
  const todayCount = await AnalysisRecord.count({
    where: {
      teacherId,
...
// 主函数：执行AI分析
export async function performAIAnalysis(analysisId: string) {
  try {
    // 1. 获取分析记录
    const analysisRecord = await AnalysisRecord.findByPk(analysisId);
    if (!analysisRecord) {
    ...
    const prompt = buildAnalysisPrompt(
      desensitizedData,
      dimensions,
      analysisRecord.customInstruction,
      courseInfo
    );
...
// 获取教师分析统计
export async function getTeacherAnalysisStats(teacherId: string) {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
